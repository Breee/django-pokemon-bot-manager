...
class GeofenceHelper(object):
    def __init__(self, geofencefile=None, geofence_dict=None):
        ...
        if geofencefile:
            ...
            coordinates_to_points = [Point(float(lat), float(lon)) for lat,lon in geofence_dict['coordinates']]
            self.geofences[geofence_dict['name']] = Polygon([(c.x, c.y) for c in coordinates_to_points])
